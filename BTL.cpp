...
void Xuat(NV x){
	fflush(stdin);
     printf("\n ",x.name,x.province,x.age,x.msnv,x.truong);
}
void XuatN(danhsach ds){
	printf("\n                  Xuat danh sach nhan vien                          \n");
    printf("\n____________________________________________________________________\n");
    printf("%-20s %-20s %-15s %15s  %15s \n", "Ho Ten "," Tinh"," Tuoi","Ma So Nhan Vien","Truong");
    for(int i = 0 ; i< ds.n ; i++){
    	Xuat(ds.a[i]);
	}
	printf("\n___________________________________________________________________\n");	
...
void xuatFile(danhsach ds, char fileName[]){
    ...
    for(int i = 0;i < ds.n;i++){
        fprintf(fp, "\n%-20s %-20s %-20d %-20d %\n", ds.a[i].name, ds.a[i].province, ds.a[i].age, ds.a[i].msnv);
    }
    fclose (fp);
}
void menu(){
